...
in `src/rust/Cargo.toml` with features `single-cell` + `multi-modal`. Check
whether that dependency is a published version or a local `path = ...` override
before building: branches under active development often point at a sibling
