...
#'
#' @return data.table with go_id and a list_col of collapsed, associated ensembl identifiers.
#'
#' @export
#'
#' @import data.table
#' @import foreach
#' @importFrom magrittr `%>%`
.get_go_genes_biomind <- function(dt) {
  # Checks
...
#'
#' @return A list with two data.tables with the first containing the depth information
#' and the second the ancestry information
#'
#' @export
#'
#' @import data.table
#' @importFrom magrittr `%>%`
.get_go_ontology_depth_ancestry_biomind <- function(dt) {
  # Checks
  checkmate::assertDataTable(dt)
  # Function body
  ontology <- dt %>%
  ...
  dist_to_nodes <-
    matrixStats::rowMaxs(dist_to_nodes_combined, na.rm = T)
  dist_to_nodes[is.infinite(dist_to_nodes)] <- 0

  level_df <- data.table(go_id = names(dist_to_nodes),
                         depth = dist_to_nodes)
...
#'
#' The function needs to be pointed to the processed BioMind data and will automatically
#' ingest and process the gene ontology data for usage for GSE with elimination method.
#'
#' @param path_to_biomind_processed The path to the processed BioMind data with the .parquet files in there
#'
#' @return A data.table with all of the data ready for ingestion into the S7_GO_obj.
#'
#' @export
#'
#' @import data.table
