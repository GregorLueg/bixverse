...
checkKnnParams <- function(x, required_params = NULL) {
    ...
    return(
      sprintf(
        paste(
          ...
          "and n_tables must be >= 1; search_budget, ef_budget,",
          "max_candidates, n_probes and n_centroids must be NULL or >= 1."
        ),
        broken_elem
      )
    )
  ...
  # choice rules
  test_choice_rules <- list(
    knn_method = c("annoy", "hnsw", "nndescent", "lsh", "exhaustive", "ivf"),
