# SpatialSpot S7 and sp_cache and sp_sample S3 ---------------------------------
...
#'
#' @description
#' Helper to build the `SpatialSample` S3 class. Stores per-experiment spatial
#' metadata: technology, number of spots, the obs column names holding
#' coordinates, the parsed scale factors and (relative) paths to the images on
#' disk. Image paths are expected to be RELATIVE to the SpatialSpot `dir_data`
#' so the directory is portable.
#'
...
#' units. Defaults to the Visium full-resolution columns.
#' @param array_cols Named character vector of length 2 with names
#' `c("row", "col")` giving the obs column names with array (grid) coordinates.
#' Visium specific; can be empty for Xenium.
#' @param scale_factors Named list. Parsed `scalefactors_json.json` contents for
#' Visium; named list of relevant scaling info for Xenium. Validated via
