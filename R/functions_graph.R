...
#' same length as `from`.
#' @param labels Character vector. The labels with `NA` indicating unlabelled.
#' @param weights Optional numeric vector. The weights between the nodes. Must
#' be the same length as `from`. If not provided, all edges are treated as
#' unweighted.
#' @param label_prop_params List. Named list of label propagation parameters as
#' generated by [params_label_propagation()]. If not provided, defaults will be
#' used.
#'
#' @return A list with the following elements:
#' \itemize{
#'   \item \code{assignment_probs} - Matrix with the assignment probabilities.
#'   \item \code{final_labels} - Final labels in the graph.
#' }
#'
#' @export
knn_graph_label_propagation <- function(
  ...
  checkmate::qassert(from, "I+")
  checkmate::qassert(to, "I+[1,]")
  checkmate::assert(
    checkmate::checkIntegerish(to, len = length(from)),
    .var.name = "to"
  )
  checkmate::qassert(labels, "s+")
  checkmate::assert(
    checkmate::test_numeric(weights, len = length(from)),
    checkmate::test_null(weights)
  )
  assertLabelPropParams(label_prop_params)
  ...
  res <- rs_knn_label_propagation(
    ...
    weights = weights,
    label_prop_params = label_prop_params
  )

  colnames(res) <- colnames(one_hot_encoding)
  row_maxes <- apply(res, 1, max)
  final_labels <- ifelse(
    row_maxes == 0,
    NA_character_,
    colnames(res)[max.col(res)]
  )

  list(assignment_probs = res, final_labels = final_labels)
}
