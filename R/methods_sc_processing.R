...
#'
#' @description
#' MAGIC smooths counts over the kNN graph: a row-stochastic diffusion operator
#' is built from the graph stored on the object and applied `n_steps` times, so
#' every cell becomes a weighted average of its neighbourhood. It makes sparse
#' marker genes readable on an embedding and gives gene trends something less
#' speckled to fit. For details, please refer to van Dijk, et al.
#'
#' Deliberately restricted to a gene subset. The output is dense and the binary
#' store is never touched, so pick the genes you want to plot or fit and impute
#' those. Genome-wide imputation is not offered.
#'
#' Each call replaces the whole layer. Read it back with
#' [bixverse::get_magic()], or hand it to any of the extractors with
#' `layer = "magic"`.
#'
#' @section What this does to your results:
...
#' inflated, badly, and the inflation is a property of the graph rather than of
#' the biology. Do not feed imputed values into Hotspot, SCENIC, differential
#' correlation or CoReMo: those methods measure exactly the quantity MAGIC
#' manufactures. Visualisation and trend fitting are what it is for.
#'
#' The operator preserves per-cell mass, so imputed values sit on the scale of
#' whatever went in. Imputing raw counts and imputing log-normalised counts are
#' different operations rather than the same one rescaled, see the `layer`
#' element of [bixverse::params_sc_magic()].
#'
#' @param object One of `SingleCells`, `SingleCellsSubset` or
#' `SingleCellsMultiModal`. `MetaCells` are already aggregated and are not
#' supported.
#' @param features Character vector. The genes to impute. Anything not found in
#' the object is dropped with a warning.
#' @param modality String. One of `c("rna", "adt", "wnn")`. Which kNN graph does
#' the smoothing. Anything but `"rna"` requires a `SingleCellsMultiModal`
#' object. The counts themselves always come from the RNA store, and the layer
#' is always written to the RNA cache.
#' @param magic_params List. See [bixverse::params_sc_magic()].
#' @param .verbose Boolean or integer. Controls verbosity and returns run times.
#' `FALSE` -> quiet, `TRUE` or `1L` -> normal verbosity, `2L` -> detailed
#' verbosity.
#'
#' @returns The object with an `ScMagic` layer in its cache.
#'
...
# method shared across SingleCells and SingleCellsSubset

S7::method(run_magic_sc, ScOrScSubset) <- function(
  object,
  features,
  modality = c("rna", "adt", "wnn"),
  magic_params = params_sc_magic(),
  ...
  # hard tier: the kNN indices and the cell selection go to Rust as they are
  ...
  # the kNN rows and the kept cells are aligned positionally
  if (length(knn_data$used_cells) != length(cell_indices)) {
    stop(sprintf(
      paste(
        "The kNN graph covers %i cells but %i are kept.",
        "Regenerate the neighbours so the two agree."
      ),
      length(knn_data$used_cells),
      length(cell_indices)
    ))
  }

  # the store header, not `dims`: on a subset the cell ids are global into the
  # parent store and the Rust side sizes its lookup against the header
  total_cells <- as.integer(get_sc_rust_ptr(object)$get_shape()[1])

  if (.verbose) {
    message(sprintf(
      "Running MAGIC over the %s kNN graph (%i cells, %i genes, %i steps).",
      modality,
      length(cell_indices),
      length(resolved$indices),
      magic_params$n_steps
    ))
  }

  imputed <- rs_magic_impute(
    f_path = get_rust_count_gene_f_path(object),
    knn_data = knn_data,
    cell_indices = cell_indices,
    total_cells = total_cells,
    gene_indices = resolved$indices,
    magic_params = magic_params,
    verbose = parse_verbosity(.verbose)
  )
