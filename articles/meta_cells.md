...
#> 2:   SEACells 0.9071613 1.0000000
...
#>        <char>           <num>             <num>            <num>
#> 1:    hdWGCNA       0.2296309         0.1619663       0.04015358
#> 2:   SEACells       0.2984881         0.2517844       0.04371741
#> 3: SuperCells       0.3235611         0.2823486       0.03296061
#>    median_compactness
#>                 <num>
#> 1:         0.02346519
#> 2:         0.02364935
...
#> 1:   SEACells   high      0.09839552        0.06426560       0.03820344
#> 2:   SEACells    mid      0.24858123        0.21486215       0.02019086
#> 3:   SEACells    low      0.40522654        0.35579760       0.06392928
#> 4: SuperCells   high      0.08978872        0.09189878       0.01214338
#> 5: SuperCells    mid      0.26108684        0.24177319       0.01418878
#> 6: SuperCells    low      0.47085047        0.45609272       0.06127352
#> 7:    hdWGCNA   high      0.06160546        0.05265773       0.02189889
#> 8:    hdWGCNA    mid      0.19093211        0.17392711       0.02312326
#> 9:    hdWGCNA    low      0.51732468        0.43812367       0.10195854
#>    median_compactness
#>                 <num>
#> 1:         0.01441045
#> 2:         0.01580934
...
#> Built 591 regulons (132 dropped below 10 genes). Median size: 319
```
...
The default is `"recovery"`, the original AUCell recovery curve, which
is what SCENIC uses. `"wilcox"` (Mann-Whitney AUC over the full ranking)
and `"ap"` (average precision) are the other two options.
...
The same argument applies to matrix factorisation, and here compression
buys you something concrete. Consensus NMF ([Kotliar, et
al.](https://elifesciences.org/articles/43803)) fits `n_runs` NMFs,
pools their components, drops the ones sitting on their own, clusters
the survivors and takes the median of each cluster. What survives is the
programme structure the restarts agree on. The catch on raw single cells
is memory: every restart is dense, and they all live at once. On a few
thousand meta cells rather than a few hundred thousand cells,
`n_runs = 50` is unremarkable. This is the path where consensus NMF is
...
against reconstruction error, keeping no factors.
...
#>   k range:          2 to 12
#>   No runs per k:    10
#>   Most stable k:    3 (stability = 0.9998)
#> 
#>         k stability best_error median_error consensus_failed n_dropped
#>     <int>     <num>      <num>        <num>           <lgcl>     <int>
#>  1:     2 0.9984253 0.16357014   0.16360325            FALSE         0
#>  2:     3 0.9998102 0.13163950   0.13164973            FALSE         0
#>  3:     4 0.9970441 0.11493415   0.11495408            FALSE         0
#>  4:     5 0.9986752 0.10272858   0.10275481            FALSE         0
#>  5:     6 0.9551063 0.09282853   0.09286200            FALSE         0
#>  6:     7 0.8888487 0.08629628   0.08633831            FALSE         0
#>  7:     8 0.8981720 0.08229806   0.08241096            FALSE         0
#>  8:     9 0.7397426 0.07847766   0.07854208            FALSE         0
#>  9:    10 0.8083084 0.07478666   0.07486889            FALSE         0
#> 10:    11 0.9458344 0.07267082   0.07275741            FALSE         0
#> 11:    12 0.7476515 0.07095402   0.07109538            FALSE         0
...
![](meta_cells_files/figure-html/nmf%20k%20sweep%20plot%20on%20meta%20cells-1.png)

Consensus NMF k sweep on the meta cells.

The usual rule is to take the last `k` before stability falls away,
while the error curve is still coming down. Here it does not fall away
so much as bounce: stability stays high across the whole range and dips
and recovers, which says the meta cells support several plausible ranks
rather than one obvious answer. It peaks at 6 and the error is still
coming down there, so that is where we fit.
...
one is a programme only some initialisations saw.

`W` (genes x k) and `H` (k x meta cells) come back in the same shape the
single-run
[`nmf_sc()`](https://gregorlueg.github.io/bixverse/reference/nmf_sc.md)
gives you, so the activations drop straight onto the UMAP:
...
![](meta_cells_files/figure-html/consensus%20nmf%20on%20the%20umap-1.png)

Two consensus programmes projected onto the meta cell UMAP.
...
top_genes[1:3]
#> $comp_01
#>  [1] "NKAIN2"  "MSI2"    "CALN1"   "ZNF385D" "MEIS1"   "INPP4B"  "ANGPT1" 
#>  [8] "CHRM3"   "ATP8B4"  "ZBTB20" 
#> 
#> $comp_02
#>  [1] "MPO"    "ATP8B4" "LRMDA"  "FNDC3B" "AZU1"   "KCNQ5"  "EREG"   "ELANE" 
#>  [9] "LYST"   "CSF3R" 
#> 
#> $comp_03
#>  [1] "DIAPH3" "ASPM"   "TOP2A"  "POLQ"   "MKI67"  "RRM2"   "CIT"    "NUSAP1"
#>  [9] "KIF15"  "AURKB"
```
...
here the standard bulk DGE pipeline applies: transpose it and hand it to
...
both in Rust, no edgeR or limma needed.
