...
With the release of `0.3.0` a lot has happened. The lack of updates had
a (big) reason… Some cooking has been going on… Since `0.3.0` the
package contains a full release of the teasered single cell
functionality suite that you can use to analyse millions of cells
locally and implements already a large number of methods in this space
into Rust and exposes them to R. [Please checkout out the website of the
package for details](https://gregorlueg.github.io/bixverse/);
particularly the sections around single cell (design choices and
vignettes.)
