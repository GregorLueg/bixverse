...
go_nodes <- arrow::read_parquet(file.path(
...
go_to_genes_clean <- go_to_genes[from %like% "ENSG" & to %like% "GO:"] %>%
  .[, .(collapsed = list(unique(from))), to] %>%
  setnames(., c("to", "collapsed"), c("go_id", "ensembl_id"))

ontology_clean <- ontology[from %like% "GO:" & to %like% "GO:", c("from", "to")]
...
dist_to_nodes_combined <- do.call(
  cbind,
  purrr::map(dist_to_nodes, ~ .$dist)
) %>%
  `colnames<-`(key_nodes)
...
ancestor_DT <- go_ontology_igraph %>%
  igraph::ego(order = igraph::vcount(go_ontology_igraph), mode = "out") %>%
