...
overdispersion. Run it on meta cells and the cell-level term becomes the
spread between aggregates within a subject rather than between cells, so
it is smaller and absorbs whatever the aggregation smoothed away. The
subject-level term keeps its meaning either way.
...
- reml:
  ...
  Rust port does not implement, so this arm has not been validated
  against an R reference. Leave it off unless you know why you want it.
  Defaults to `FALSE`.
