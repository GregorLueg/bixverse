...
```{r}
scrublet_obs <- get_data(scrublet)
...
```{r}
get_sc_var(sc_object)[gene_name == "CD3D"]
```
...
sc_object <- umap_sc(
  sc_object,
  modality = "rna",
  umap_params = manifoldsR::params_umap(init = "random")
)
...
get_sc_var(sc_object, modality = 'adt')[["feature_id"]]
