...
```{r}
#| label: calculate the mutual informations on a per column basis
#| eval: !expr requireNamespace("infotheo", quietly = TRUE)

rust_res_mi <- rs_mutual_info(mat, n_bins = NULL, normalise = FALSE)
...
```{r}
#| label: speed differences bixverse vs. infotheo
#| eval: !expr requireNamespace(c("infotheo", "microbench"), quietly = TRUE)
...
microbenchmark::microbenchmark(
  ...
  rust = rs_mutual_info(mat, n_bins = NULL, normalise = FALSE),
  times = 50L
)
```
...
Let's start with a naive purrr approach. Nonetheless, we can appreciate already
that some type of parallelism will help here. But for completeness, let's test
purrr...
...
Let's try to parallelise this via future and furrr back-ends and see how 
much we can improve this.
...
in `bixverse` and can be integrated into other workflows.
