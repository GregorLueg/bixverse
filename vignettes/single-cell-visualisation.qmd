...
This vignette gives an overview of the different plotting functions that are available when running a single cell analysis. 
...
We run the standard analysis using the same PBMC3k data set. For more details on the data analysis itself, please refer to the [introductory vignette](https://gregorlueg.github.io/bixverse/articles/thinking_single_cell.html). This vignette assumes familiarity with how the `SingleCells` class, on-disk storage and cells-to-keep logic work.
...
`joint_plot_sc()` compares the number of features and number of counts per cell similar to `sns.jointplot`.
...
Alternatively, we can also provide a datatable generate the violins plots with different variables than those considered in the QC.
...
`density_plot_sc()` overlays smoothed density distributionss for each group, which
...
on any 2D embedding stored in the object.
...
overlay text labels at the cluster centroid. These do not need to be the same variables.
...
Alternatively, you can also extract the plotting data directly to make customized plots.
...
`feature_plot_sc()` projects normalised expression of one or more genes
onto the embedding.
...
We can also highlight the genes of interest for clearer visualization.
