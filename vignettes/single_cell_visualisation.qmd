...
```{r joint-plot, fig.cap="Joint QC plot showing features versus counts."}
joint_plot_sc(qc)
```
...
`density_plot_sc()` overlays smoothed density distributionss for each group, 
which highlights bimodal distributions or outlier populations that violins can 
obscure.

```{r density-plot, fig.cap="Density plots for each QC metric."}
density_plot_sc(qc)
```
...
```{r embedding-sctype, fig.cap="UMAP coloured by ribosomal content (continuous scale)."}
embedding_plot_sc(
  sc_object,
  embedding = "umap",
  colour_by = "Ribo",
...
```{r feature-plot, fig.cap="Expression of MS4A1 on the UMAP. High-expressing cells cluster in expected B cell cluster."}
...
```{r feature-plot-higlight, fig.cap="Expression of MS4A1 on the UMAP. High-expressing cells cluster in expected B cell cluster."}
# Select a single feature by Ensembl ID; provide a human-readable label
features <- setdiff(symbol_to_ensembl[names(cell_markers)], NA)
feature_labels <- ensembl_to_symbol[features]
...
```{r stacked-violin, fig.cap="Stacked violin plot of markers across final cell types."}
